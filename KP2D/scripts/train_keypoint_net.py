...
def main(file):
    ...
    #keypoint_net = loadModel("/mnt/g27prist/TCO/TCO-Studenten/ariel_research_project/pretrained_models/kp2d/v4.ckpt")#KeypointNetwithIOLoss(**config.model.params)
    model = KeypointNetwithIOLoss(keypoint_net=None, **config.model.params)
    ...
    output_path = os.path.join(config.model.checkpoint_path, "scratch")
    ...
    if rank() == 0:
        if not config.wandb.dry_run:
            ...
            config.model.checkpoint_path = output_path
    ...
    # Initial evaluation
    evaluation(config, 0, model, summary, logs)
    # Train
    for epoch in range(config.arch.epochs):
        ...
        evaluation(config, epoch + 1, model, summary, logs)
    df = pd.DataFrame(logs, columns=['epoch', 'repeatability', 'localization_error', 'correctness_d1', 'correctness_d3', 'correctness_d5', 'mscore'])
    df.to_csv(str(Path(output_path, 'logs.csv')))
    printcolor('Training complete, models saved in {}'.format(config.model.checkpoint_path), "green")


def evaluation(config, completed_epoch, model, summary, logs):
    # Set to eval mode
    model.eval()
    ...
    if rank() == 0:
        ...
        for params in eval_params:
            ...
            logs.append([completed_epoch, rep, loc, c1, c3, c5, mscore])
    ...
    if config.model.save_checkpoint and rank() == 0:
        current_model_path = os.path.join(config.model.checkpoint_path, f'{completed_epoch}.ckpt')
